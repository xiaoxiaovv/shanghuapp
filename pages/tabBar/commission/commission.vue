	...
	<view @click="closeCover">
		...
		<mescroll-uni @down="downCallback" :up="mescrollUp" @up="upCallback" @init="mescrollInit">
			<view 
				...
				<view v-for="order in orderList" :key="order.id" class="lf-orderList match-width align-left ptb-10">
					...
					<view class="match-left-space align-ver-left"> 			
						<view class="match-width align-hor-bet">
							...
							<view class="">
								...
								<view class="mt-4 ly-font-color-9">{{order.payWay | payWayFilters}}</view>
							</view>
							<view class="">
								<view class="align-right">{{yenSymbol}} {{(order.userCommission)}}</view>
								<view class="mt-4 ly-font-color-9">{{order.createDate}}</view>
							</view>
...
<script>
	import { getcommissionList, queryOrderSummary } from '@/api/vueAPI.js'
	...
	export default {
		...
		methods:{
			...
			upCallback(mescroll) {
				// console.log("测试点-1")
				this.getcommissionList( mescroll.num, mescroll.size, (res) => {
					console.log("测试点1");
					console.log(res);
			...
			/* 获取订单列表 */
			getcommissionList(pageNumber, pageSize, successCallback, errorCallback) {
