...
var config = {
    ...
    columePadding: 3,
    fontSize: 13,
    //valuePointShape: ['diamond', 'circle', 'triangle', 'rect'],
	valuePointShape: ['circle', 'circle', 'circle', 'circle'],//仿F2图例样式改为圆点
    colors: ['#2fc25b', '#1890ff', '#facc14', '#f04864', '#8543e0'],
...
function findRange(num, type, limit) {
    if (isNaN(num)) {
        throw new Error('[wxCharts] unvalid series value!');
    }
    limit = limit || 10;
...
function calValidDistance(distance, chartvalue, config, opts) {

    var valueChartAreaWidth = opts.width - config.padding - chartvalue.xAxisPoints[0];
    var valueChartWidth = chartvalue.eachSpacing * opts.categories.length;
    var validDistance = distance;
    if (distance >= 0) {
        validDistance = 0;
    } else if (Math.abs(distance) >= valueChartWidth - valueChartAreaWidth) {
        validDistance = valueChartAreaWidth - valueChartWidth;
    }
...
function getvalueRange(minvalue, maxvalue) {
    var limit = 0;
    ...
    return {
        minRange: findRange(minvalue, 'lower', limit),
        maxRange: findRange(maxvalue, 'upper', limit)
    };
}
...
function valueCombine(series) {
    return series.reduce(function (a, b) {
        return (a.value ? a.value : a).concat(b.value);
    }, []);
}

function valueCombineStack(series) {
	var sum = new Array(series[0].value.length);
	for(var j = 0; j < sum.length; j++) {
			sum[j] =0;
	}
	for(var i = 0; i < series.length; i++) {
	  for(var j = 0; j < sum.length; j++) {
		sum[j] += series[i].value[j];
	  }
	}
	return series.reduce(function (a, b) {
	    return (a.value ? a.value : a).concat(b.value).concat(sum);
	}, []);
}
...
function getSeriesvalueItem(series, index) {
    var value = [];
    series.forEach(function (item) {
        if (item.value[index] !== null && typeof item.value[index] !== 'undefined') {
            var seriesItem = {};
            seriesItem.color = item.color;
            seriesItem.name = item.name;
            seriesItem.value = item.format ? item.format(item.value[index]) : item.value[index];
            value.push(seriesItem);
        }
    });
...
function getToolTipvalue(seriesvalue, calPoints, index, categories) {
    var option = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {};

    var textList = seriesvalue.map(function (item) {
        return {
            text: option.format ? option.format(item, categories[index]) : item.name + ': ' + item.value,
            color: item.color
        };
    });
    var validCalPoints = [];
...
function getCandleToolTipvalue(series,seriesvalue, calPoints, index, categories,extra) {
	...
	textList.push(text0);
	seriesvalue.map(function (item) {
		//console.log(color)
		if(index==0 && item.value[1]-item.value[0]<0){
			color[1]=downColor;
		}else{
			if(item.value[0]<series[index-1][1]){
				color[0]=downColor;
			}
			if(item.value[1]<item.value[0]){
				color[1]=downColor;
			}
			if(item.value[2]>series[index-1][1]){
				color[2]=upColor;
			}
			if(item.value[3]<series[index-1][1]){
				color[3]=downColor;
			}
		}
		
		let text1={
			text: '开盘：'+item.value[0],
			color: color[0]
		};
		let text2={
			text: '收盘：'+item.value[1],
			color: color[1]
		};
		let text3={
			text: '最低：'+item.value[2],
			color: color[2]
		};
		let text4={
			text: '最高：'+item.value[3],
			color: color[3]
		};
...
function findRadarChartCurrentIndex(currentPoints, radarvalue, count) {
    var eachAngleArea = 2 * Math.PI / count;
    var currentIndex = -1;
    if (isInExactPieChartArea(currentPoints, radarvalue.center, radarvalue.radius)) {
        ...
        var angle = Math.atan2(radarvalue.center.y - currentPoints.y, currentPoints.x - radarvalue.center.x);
        angle = -1 * angle;
        if (angle < 0) {
            angle += 2 * Math.PI;
        }

        var angleList = radarvalue.angleList.map(function (item) {
...
function findPieChartCurrentIndex(currentPoints, pievalue) {
    var currentIndex = -1;
    if (isInExactPieChartArea(currentPoints, pievalue.center, pievalue.radius)) {
        var angle = Math.atan2(pievalue.center.y - currentPoints.y, currentPoints.x - pievalue.center.x);
        angle = -angle;
        for (var i = 0, len = pievalue.series.length; i < len; i++) {
            var item = pievalue.series[i];
...
function calLegendvalue(series, opts, config) {
...
function calCategoriesvalue(categories, opts, config) {
    var result = {
        angle: 0,
...
function getRadarvaluePoints(angleList, center, radius, series, opts) {
    var process = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 1;

    var radarOption = opts.extra.radar || {};
    radarOption.max = radarOption.max || 0;
    var maxvalue = Math.max(radarOption.max, Math.max.apply(null, valueCombine(series)));
    ...
    series.forEach(function (each) {
        ...
        listItem.value = [];
        each.value.forEach(function (item, index) {
            var tmp = {};
            tmp.angle = angleList[index];

            tmp.proportion = item / maxvalue;
            tmp.position = convertCoordinateOrigin(radius * tmp.proportion * process * Math.cos(tmp.angle), radius * tmp.proportion * process * Math.sin(tmp.angle), center);
            listItem.value.push(tmp);
        });
...
function getPievaluePoints(series) {
    var process = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;

    var count = 0;
    var _start_ = 0;
    series.forEach(function (item) {
        item.value = item.value === null ? 0 : item.value;
        count += item.value;
    });
    series.forEach(function (item) {
        item.value = item.value === null ? 0 : item.value;
		item._proportion_ = item.value / count * process;	
    });
    series.forEach(function (item) {
...
function getArcbarvaluePoints(series,arcbarOption) {
    ...
    series.forEach(function (item) {
		...
		item._proportion_ = totalAngle * item.value* process + arcbarOption.startAngle;
...
function getGaugevaluePoints(series,categories,gaugeOption) {
    var process = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 1;
    series.forEach(function (item) {
        item.value = item.value === null ? 0 : item.value;
		if(gaugeOption.pointer.color=='auto'){
			for(let i=0 ;i<categories.length;i++){
				if(item.value<=categories[i].value){
		...
		item._endAngle_=totalAngle * item.value + gaugeOption.startAngle;
		item._oldAngle_=gaugeOption.oldAngle;
		if(gaugeOption.oldAngle<gaugeOption.endAngle){
			item._oldAngle_+=2;
		}
		if(item.value>=gaugeOption.oldvalue){
...
function getPieTextMaxLength(series) {
    series = getPievaluePoints(series);
    var maxLength = 0;
    series.forEach(function (item) {
...
function fixColumevalue(points, eachSpacing, columnLen, index, config, opts) {
...
function fixColumeMetervalue(points, eachSpacing, columnLen, index, config, opts, border) {
    return points.map(function (item) {
        if (item === null) {
            return null;
        }
...
function fixColumeStackvalue(points, eachSpacing, columnLen, index, config, opts,series) {
...
function getXAxisPoints(categories, opts, config) {
    ...
    var valueCount = opts.enableScroll ? Math.min(opts.xAxis.itemCount, categories.length) : categories.length;
    var eachSpacing = spacingValid / valueCount;
...
function getCandlevaluePoints(value, minRange, maxRange, xAxisPoints, eachSpacing, opts, config) {
...
function getvaluePoints(value, minRange, maxRange, xAxisPoints, eachSpacing, opts, config) {
    var process = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : 1;
...
function getStackvaluePoints(value, minRange, maxRange, xAxisPoints, eachSpacing, opts, config,seriesIndex, stackSeries) {
        ...
        if (item === null) {
			...
			if(seriesIndex>0){
				...
				for(let i=0;i<=seriesIndex;i++){
					value+=stackSeries[i].value[index];
				}
...
function getYAxisTextList(series, opts, config ,stack) {
	var value;
	if(stack=='stack'){
		//value = valueCombine(series);
		value = valueCombineStack(series);
	}else{
		value = valueCombine(series);
	}
    ...
    //var minvalue = Math.min.apply(this, value);
    //var maxvalue = Math.max.apply(this, value);
	value.map((item)=>{
	...
	var maxvalue = 0;
	if(sorted.length>0){
		minvalue = Math.min.apply(this, sorted);
		maxvalue = Math.max.apply(this, sorted);
	}
    if (typeof opts.yAxis.min === 'number') {
       minvalue = Math.min(opts.yAxis.min, minvalue);
    }
    if (typeof opts.yAxis.max === 'number') {
        maxvalue = Math.max(opts.yAxis.max, maxvalue);
    }

    // fix issue https://github.com/xiaolin3303/wx-charts/issues/9
    if (minvalue === maxvalue) {
        var rangeSpan = maxvalue || 10;
        //minvalue -= rangeSpan;
        maxvalue += rangeSpan;
    }

    var valueRange = getvalueRange(minvalue, maxvalue);
    var minRange = valueRange.minRange;
    var maxRange = valueRange.maxRange;
...
function calYAxisvalue(series, opts, config) {
	//堆叠图重算Y轴
...
function calTooltipYAxisvalue(point,series, opts, config, eachSpacing) {
...
function drawPointText(points, series, config, context) {
    ...
    points.forEach(function (item, index) {
        if (item !== null) {
            //var formatVal = series.format ? series.format(value[index]) : value[index];
			context.beginPath();
...
function drawToolTipHorizentalLine(opts, config, context, eachSpacing, xAxisPoints) {
	...
	if(toolTipOption.yAxisLabel){
		...
		let lableText = calTooltipYAxisvalue(opts.tooltip.offset.y,opts.series, opts, config,eachSpacing);
		context.setFontSize(config.fontSize);
		let textWidth = context.measureText(lableText).width;
...
function drawColumnvaluePoints(series, opts, config, context) {
    ...
    var _calYAxisvalue = calYAxisvalue(series, opts, config),
        ranges = _calYAxisvalue.ranges;
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        var value = eachSeries.value;
		switch (columnOption.type){
		case 'group':
			var points = getvaluePoints(value, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, process);
			var tooltipPoints = getStackvaluePoints(value, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, seriesIndex, series,process);
			calPoints.push(tooltipPoints);
			points = fixColumevalue(points, eachSpacing, series.length, seriesIndex, config, opts);
			points.forEach(function (item, index) {
				if (item !== null) {
					context.beginPath();
		...
		case 'stack':
			// 绘制堆叠数据图
			var points = getStackvaluePoints(value, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, seriesIndex, series,process);
			calPoints.push(points);
			points = fixColumeStackvalue(points, eachSpacing, series.length, seriesIndex, config, opts,series);
		...
		// 绘制温度计数据图
			var points = getvaluePoints(value, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, process);
			calPoints.push(points);
			points = fixColumeMetervalue(points, eachSpacing, series.length, seriesIndex, config, opts, columnOption.meter.border);
			if(seriesIndex==0){
				points.forEach(function (item, index) {
					if (item !== null) {
						//画背景颜色
	...
	if (opts.valueLabel !== false && process === 1) {
	    series.forEach(function (eachSeries, seriesIndex) {
	        var value = eachSeries.value;
			switch (columnOption.type){
				case 'group':
...
function drawCandlevaluePoints(series, opts, config, context) {
    var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
    ...
    var _calYAxisvalue5 = calYAxisvalue(series, opts, config),
        ranges = _calYAxisvalue5.ranges;
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        var value = eachSeries.value;
        var points = getCandlevaluePoints(value, minRange, maxRange, xAxisPoints, eachSpacing, opts, config, process);
        calPoints.push(points);
        ...
        splitPointList.forEach(function (points, index) {
			context.beginPath();
			//如果上涨
			if(value[index][1]-value[index][0]>0){
...
function drawAreavaluePoints(series, opts, config, context) {
    var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
    ...
    var _calYAxisvalue2 = calYAxisvalue(series, opts, config),
        ranges = _calYAxisvalue2.ranges;
    ...
    series.forEach(function (eachSeries, seriesIndex) {
		...
		//画点
        if (opts.valuePointShape !== false) {
            var shape = config.valuePointShape[seriesIndex % config.valuePointShape.length];
...
function drawLinevaluePoints(series, opts, config, context) {
    var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
    ...
    var _calYAxisvalue3 = calYAxisvalue(series, opts, config),
        ranges = _calYAxisvalue3.ranges;
...
function drawMixvaluePoints(series, opts, config, context) {
    var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;

    var _calYAxisvalue6 = calYAxisvalue(series, opts, config),
        ranges = _calYAxisvalue6.ranges;
    ...
    series.forEach(function (eachSeries, seriesIndex) {
		...
		// 绘制柱状数据图
		if(eachSeries.type=='column'){
			points = fixColumevalue(points, eachSpacing, columnLength , columnIndex, config, opts);
			points.forEach(function (item, index) {
			    if (item !== null) {
					context.beginPath();
					context.setFillStyle(item.color || eachSeries.color);
        ...
        if (opts.valuePointShape !== false && eachSeries.type!=='column') {
            var shape = config.valuePointShape[seriesIndex % config.valuePointShape.length];
            drawPointShape(points, eachSeries.color, shape, context,opts);
        }
    });
...
function drawYAxis(series, opts, config, context) {
    ...
    var _calYAxisvalue4 = calYAxisvalue(series, opts, config),
        rangesFormat = _calYAxisvalue4.rangesFormat;
...
function drawLegend(series, opts, config, context) {
    ...
    var _calLegendvalue = calLegendvalue(series, opts, config),
        legendList = _calLegendvalue.legendList;
...
function drawPievaluePoints(series, opts, config, context) {
    var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;

    var pieOption = opts.extra.pie || {};
    series = getPievaluePoints(series, process);
    var centerPosition = {
        x: opts.width / 2,
...
function drawArcbarvaluePoints(series, opts, config, context) {
    var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
    ...
    series = getArcbarvaluePoints(series, arcbarOption, process);
    var centerPosition = {
        x: opts.width / 2,
        y: (opts.height) / 2
    };
...
function drawGaugevaluePoints(categories,series, opts, config, context) {
	var process = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 1;
	...
	if(gaugeOption.oldvalue==undefined){
		gaugeOption.oldvalue=0;
	}
	...
	series = getGaugevaluePoints(series,categories,gaugeOption, process);
	...
	if (process === 1 && opts.type === 'gauge') {
		...
		gaugeOption.oldvalue=series[0].value;
	}
	return {
	    center: centerPosition,
	    radius: radius,
...
function drawRadarvaluePoints(series, opts, config, context) {
    var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
    ...
    var radarvaluePoints = getRadarvaluePoints(coordinateAngle, centerPosition, radius, series, opts, process);
	
    radarvaluePoints.forEach(function (eachSeries, seriesIndex) {
        // 绘制区域数据
        context.beginPath();
        context.setFillStyle(eachSeries.color);
        context.setGlobalAlpha(0.3);
        eachSeries.value.forEach(function (item, index) {
            if (index === 0) {
        ...
        if (opts.valuePointShape !== false) {
            var shape = config.valuePointShape[seriesIndex % config.valuePointShape.length];
            var points = eachSeries.value.map(function (item) {
...
function drawCharts(type, opts, config, context) {
    ...
    var _calLegendvalue = calLegendvalue(series, opts, config),
        legendHeight = _calLegendvalue.legendHeight;

    config.legendHeight = legendHeight;

    var _calYAxisvalue = calYAxisvalue(series, opts, config),
        yAxisWidth = _calYAxisvalue.yAxisWidth;

    config.yAxisWidth = yAxisWidth;
    if (categories && categories.length) {
        var _calCategoriesvalue = calCategoriesvalue(categories, opts, config),
            xAxisHeight = _calCategoriesvalue.xAxisHeight,
            angle = _calCategoriesvalue.angle;

        config.xAxisHeight = xAxisHeight;
        config._xAxisTextAngle_ = angle;
    }
    if (type === 'pie' || type === 'ring') {
        config._pieTextMaxLength_ = opts.valueLabel === false ? 0 : getPieTextMaxLength(series);
    }
        ...
        case 'line':
            this.animationInstance = new Animation({
                ...
                onProcess: function onProcess(process) {
                    ...
                    var _drawLinevaluePoints = drawLinevaluePoints(series, opts, config, context, process),
                        xAxisPoints = _drawLinevaluePoints.xAxisPoints,
                        calPoints = _drawLinevaluePoints.calPoints,
                        eachSpacing = _drawLinevaluePoints.eachSpacing;

                    _this.chartvalue.xAxisPoints = xAxisPoints;
                    _this.chartvalue.calPoints = calPoints;
                    _this.chartvalue.eachSpacing = eachSpacing;
		...
		case 'mix':
		    this.animationInstance = new Animation({
		        ...
		        onProcess: function onProcess(process) {
		            ...
		            drawYAxisGrid(categories,opts, config, context);
		            drawXAxis(categories, opts, config, context);
		            var _drawMixvaluePoints = drawMixvaluePoints(series, opts, config, context, process),
		                xAxisPoints = _drawMixvaluePoints.xAxisPoints,
		                calPoints = _drawMixvaluePoints.calPoints,
		                eachSpacing = _drawMixvaluePoints.eachSpacing;
        ...
        case 'column':
		    this.animationInstance = new Animation({
                ...
                onProcess: function onProcess(process) {
					...
					drawYAxisGrid(categories,opts, config, context);
                    drawXAxis(categories, opts, config, context);
                    var _drawColumnvaluePoints = drawColumnvaluePoints(series, opts, config, context, process),
                        xAxisPoints = _drawColumnvaluePoints.xAxisPoints,
						calPoints = _drawColumnvaluePoints.calPoints,
                        eachSpacing = _drawColumnvaluePoints.eachSpacing;
                    _this.chartvalue.xAxisPoints = xAxisPoints;
					_this.chartvalue.calPoints = calPoints;
                    _this.chartvalue.eachSpacing = eachSpacing;
                    drawLegend(opts.series, opts, config, context);
        ...
        case 'area':
            this.animationInstance = new Animation({
                ...
                onProcess: function onProcess(process) {
					...
					drawYAxisGrid(categories,opts, config, context);
                    drawXAxis(categories, opts, config, context);
                    var _drawAreavaluePoints = drawAreavaluePoints(series, opts, config, context, process),
                        xAxisPoints = _drawAreavaluePoints.xAxisPoints,
                        calPoints = _drawAreavaluePoints.calPoints,
                        eachSpacing = _drawAreavaluePoints.eachSpacing;
        ...
        case 'pie':
            this.animationInstance = new Animation({
                ...
                onProcess: function onProcess(process) {
					...
					_this.chartvalue.pievalue = drawPievaluePoints(series, opts, config, context, process);
                    drawLegend(opts.series, opts, config, context);
        ...
        case 'radar':
            this.animationInstance = new Animation({
                ...
                onProcess: function onProcess(process) {
					...
					_this.chartvalue.radarvalue = drawRadarvaluePoints(series, opts, config, context, process);
                    drawLegend(opts.series, opts, config, context);
                    drawCanvas(opts, context);
                },
                onAnimationFinish: function onAnimationFinish() {
		...
		case 'arcbar':
			this.animationInstance = new Animation({
			    ...
			    onProcess: function onProcess(process) {
					...
					_this.chartvalue.arcbarvalue = drawArcbarvaluePoints(series, opts, config, context, process);
			        drawCanvas(opts, context);
			    },
			    onAnimationFinish: function onAnimationFinish() {
			        _this.event.trigger('renderComplete');
		...
		case 'gauge':
			this.animationInstance = new Animation({
			    ...
			    onProcess: function onProcess(process) {
					...
					_this.chartvalue.gaugevalue = drawGaugevaluePoints(categories, series,opts, config, context, process);
			        drawCanvas(opts, context);
			    },
			    onAnimationFinish: function onAnimationFinish() {
			        _this.event.trigger('renderComplete');
		...
		case 'candle':
		    this.animationInstance = new Animation({
		        ...
		        onProcess: function onProcess(process) {
					...
					drawYAxisGrid(categories,opts, config, context);
					drawXAxis(categories, opts, config, context);
					var _drawCandlevaluePoints = drawCandlevaluePoints(series, opts, config, context, process),
					    xAxisPoints = _drawCandlevaluePoints.xAxisPoints,
					    calPoints = _drawCandlevaluePoints.calPoints,
					    eachSpacing = _drawCandlevaluePoints.eachSpacing;
...
var Charts = function Charts(opts) {
    ...
    if(opts.type == 'pie' || opts.type == 'ring' ){
    	 config$$1.pieChartLinePadding = opts.valueLabel === false ? 0 : opts.extra.pie.lableWidth || config$$1.pieChartLinePadding*opts.pixelRatio;
    }
    config$$1.pieChartTextPadding = opts.valueLabel === false ? 0 : config$$1.pieChartTextPadding*opts.pixelRatio;
	...
	if(opts.enableScroll && opts.xAxis.scrollAlign=='right'){
		let _calYAxisvalue = calYAxisvalue(opts.series, opts, config$$1),
		    yAxisWidth = _calYAxisvalue.yAxisWidth;
...
Charts.prototype.updatevalue = function () {
    var value = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    this.opts.series = value.series || this.opts.series;
    this.opts.categories = value.categories || this.opts.categories;

    this.opts.title = assign({}, this.opts.title, value.title || {});
    this.opts.subtitle = assign({}, this.opts.subtitle, value.subtitle || {});
...
Charts.prototype.getCurrentvalueIndex = function (e) {
    var touches= e.mp.changedTouches[0];
    if (touches) {
        var _touches$= getTouches(touches, this.opts, e);
        if (this.opts.type === 'pie' || this.opts.type === 'ring') {
            return findPieChartCurrentIndex({ x: _touches$.x, y: _touches$.y }, this.chartvalue.pievalue);
        } else if (this.opts.type === 'radar') {
            return findRadarChartCurrentIndex({ x: _touches$.x, y: _touches$.y }, this.chartvalue.radarvalue, this.opts.categories.length);
        } else {
            return findCurrentIndex({ x: _touches$.x, y: _touches$.y }, this.chartvalue.xAxisPoints, this.opts, this.config, Math.abs(this.scrollOption.currentOffset));
        }
    }
    return -1;
};
...
Charts.prototype.showToolTip = function (e) {
    ...
    if (this.opts.type === 'line' || this.opts.type === 'area' || this.opts.type === 'mix' || this.opts.type === 'column') {
        var index = this.getCurrentvalueIndex(e);
        ...
        if (index > -1) {
            ...
            var seriesvalue = getSeriesvalueItem(this.opts.series, index);
            if (seriesvalue.length !== 0) {
                var _getToolTipvalue = getToolTipvalue(seriesvalue, this.chartvalue.calPoints, index, this.opts.categories, option),
                    textList = _getToolTipvalue.textList,
                    offset = _getToolTipvalue.offset;
				offset.y=_touches$.y;
                opts.tooltip = {
                    textList: textList,
                    offset: offset,
	...
	if (this.opts.type === 'candle') {
	    ...
	    if (index > -1) {
	        var seriesvalue = getSeriesvalueItem(this.opts.series, index);
	        if (seriesvalue.length !== 0) {
	            var _getToolTipvalue = getCandleToolTipvalue(this.opts.series[0].value,seriesvalue, this.chartvalue.calPoints, index, this.opts.categories, this.opts.extra.candle, option),
	                textList = _getToolTipvalue.textList,
	                offset = _getToolTipvalue.offset;
				offset.y=_touches$.y;
	            opts.tooltip = {
	...
	if (this.opts.type === 'pie' || this.opts.type === 'ring') {
	    ...
	    if (index > -1) {
	        ...
	        var seriesvalue = this.opts.series[index];
			var textList =[{
				text: option.format ? option.format(seriesvalue) : seriesvalue.name + ': ' + seriesvalue.value,
...
Charts.prototype.scroll = function (e) {
    ...
    if (touches && this.opts.enableScroll === true) {
        ...
        var validDistance = calValidDistance(currentOffset + _distance, this.chartvalue, this.config, this.opts);
