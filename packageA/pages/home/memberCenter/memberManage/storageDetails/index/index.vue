				...
				<view class="lf-orderList" @click="jumpRecordDetails(record.id)">
					<view class="lf-orderList-right">
						...
						<view class="lf-right-text">
							<view>
								<text class="text-llg text-weight">{{yenSymbol}}{{record.tradingMoney}}</text>
								<text class="text-grey">{{record.payStatus | statusFilters}}</text>
							</view>
							<view>
								<text class="text-sm text-grey">订单号：{{record.id}}</text>
								<text class="text-grey">{{record.createTime | createTimeFilters}}</text>
							</view>
						</view>
					</view>
				</view>
...
<script>
	import {getMemberRecordOne } from '@/api/vueAPI.js'
	...
	export default {
		...
		methods:{
			...
			jumpRecordDetails (id) {
				uni.navigateTo({
					url: '../storageDetail/storageDetail?id='+ id
				})
			},
			...
			upCallback(mescroll) {
				// console.log("测试点-1")
				this.getMemberRecordOne( mescroll.num, mescroll.size, (res) => {
			...
			/* 获取列表 */
			getMemberRecordOne(pageNumber, pageSize, successCallback, errorCallback) {
				// console.log("测试点0")
				getMemberRecordOne( pageNumber, pageSize, this.payWay, this.startTime, this.endTime, this.memberId).then((res) => {
					// console.log(res);
					// console.log(res.obj);
